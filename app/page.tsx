...
export default function FinFlowDemo() {
    ...
    const [isFeaturesSectionVisible, setIsFeaturesSectionVisible] =
        useState(false);
    ...
    const handleAnalysis = async () => {
        ...
        try {
            ...
            const steps = [
                ...
                {
                    message: "기술적 지표를 계산하고 있습니다...",
                    progress: 40,
                    delay: 1000,
                },
                {
                    message: "리스크 모델을 분석하고 있습니다...",
                    progress: 60,
                    delay: 1200,
                },
                {
                    message: "강화학습 모델을 추론하고 있습니다...",
                    progress: 80,
                    delay: 1500,
                },
                {
                    message: "포트폴리오를 최적화하고 있습니다...",
                    progress: 95,
                    delay: 800,
                },
            ];
            ...
            // 서버 응답을 프론트엔드 형태로 변환
            const allocation = data.allocation.map(
                (item: { symbol: string; weight: number }) => ({
                    stock: item.symbol,
                    percentage: (item.weight * 100).toFixed(1),
                    amount: Math.round(
                        item.weight * Number.parseInt(investmentAmount)
                    ),
                })
            );
            ...
            const metrics = [
                {
                    label: "총 수익률",
                    portfolio: `${data.metrics.total_return.toFixed(2)}%`,
                    spy: benchmarks.spy.totalReturn,
                    qqq: benchmarks.qqq.totalReturn,
                },
                {
                    label: "연간 수익률",
                    portfolio: `${data.metrics.annual_return.toFixed(2)}%`,
                    spy: benchmarks.spy.annualReturn,
                    qqq: benchmarks.qqq.annualReturn,
                },
                {
                    label: "샤프 비율",
                    portfolio: data.metrics.sharpe_ratio.toFixed(3),
                    spy: benchmarks.spy.sharpe,
                    qqq: benchmarks.qqq.sharpe,
                },
                {
                    label: "소르티노 비율",
                    portfolio: data.metrics.sortino_ratio.toFixed(3),
                    spy: benchmarks.spy.sortino,
                    qqq: benchmarks.qqq.sortino,
                },
                {
                    label: "최대 낙폭",
                    portfolio: `${data.metrics.max_drawdown.toFixed(2)}%`,
                    spy: benchmarks.spy.maxDrawdown,
                    qqq: benchmarks.qqq.maxDrawdown,
                },
                {
                    label: "변동성",
                    portfolio: `${data.metrics.volatility.toFixed(2)}%`,
                    spy: benchmarks.spy.volatility,
                    qqq: benchmarks.qqq.volatility,
                },
                {
                    label: "승률",
                    portfolio: `${data.metrics.win_rate.toFixed(1)}%`,
                    spy: benchmarks.spy.winRate,
                    qqq: benchmarks.qqq.winRate,
                },
                {
                    label: "손익비",
                    portfolio: data.metrics.profit_loss_ratio.toFixed(2),
                    spy: benchmarks.spy.profitLoss,
                    qqq: benchmarks.qqq.profitLoss,
                },
            ];
            ...
            setError(
                err instanceof Error
                    ? err.message
                    : "서버와 연결할 수 없습니다. 서버가 실행 중인지 확인해주세요."
            );
        } finally {
    ...
    const handleXAIAnalysis = async (
        method: "fast" | "accurate" = xaiMethod
    ) => {
        if (!investmentAmount) {
        ...
        console.log(
            `XAI 분석 시작 (${method} 모드, 예상 시간: ${estimatedTime})`
        );
        ...
        try {
            ...
            if (remainingTime > 0) {
                console.log(
                    `최소 대기 시간 확보를 위해 ${remainingTime}ms 추가 대기`
                );
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div
                                        className={`inline-flex items-center px-3 py-2 rounded-lg text-sm font-medium bg-gray-100 text-gray-700 border border-gray-200`}
                                    >
                                        <div
                                        ...
                                        ></div>
                                        {getRiskLevel(riskTolerance).label} 투자
                                        성향
                                    </div>
                                </div>
                                ...
                                <Button
                                    ...
                                    disabled={!investmentAmount || isAnalyzing}
            ...
            {showScrollButton && (
                <div className="flex justify-center mt-12 flex-col items-center space-y-2 text-gray-600">
                    ...
                    <button
                        ...
                        className="p-0.5 bg-white text-slate-700 hover:bg-slate-100 transition-all duration-200 cursor-pointer rounded-full"
                    >
            ...
            >
                <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <div
                            className={`text-center transition-all duration-700 delay-600 ease-out ${
                                isFeaturesSectionVisible
                                    ? "opacity-100 transform translate-y-0"
                                    : "opacity-0 transform translate-y-8"
                            }`}
            ...
            <footer className="bg-gray-900 text-white py-8">
                <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                    ...
                    <p className="text-gray-400 text-sm">
                        © 2025 FinFlow. 강화학습 기반 포트폴리오 최적화
                        플랫폼입니다.
                    </p>
                </div>
            </footer>
